# Release notes for v1.1.0

[Documentation](https://kubernetes-csi.github.io)

# Changelog since v1.0.0

## Changes by Kind

### Uncategorized

- Bump k8s dependencies to v1.36.1 ([#247](https://github.com/kubernetes-csi/external-snapshot-metadata/pull/247), [@dfajmon](https://github.com/dfajmon))
- Bump the snapshot-metadata-verifier tool dependencies to v1.0.0 for the v1beta1 API. ([#240](https://github.com/kubernetes-csi/external-snapshot-metadata/pull/240), [@dims](https://github.com/dims))
- Update release-tools to fix the gcb-docker-gcloud image used to build release images. ([#241](https://github.com/kubernetes-csi/external-snapshot-metadata/pull/241), [@Paramesh324](https://github.com/Paramesh324))
- Run the Trivy vulnerability scan on a schedule instead of on pull requests. ([#235](https://github.com/kubernetes-csi/external-snapshot-metadata/pull/235), [@torredil](https://github.com/torredil))
- Pin GitHub Actions to exact commit SHAs. ([#232](https://github.com/kubernetes-csi/external-snapshot-metadata/pull/232), [@jsafrane](https://github.com/jsafrane))
- Update trivy-action to use the pinned SHA for v0.35.0. ([#228](https://github.com/kubernetes-csi/external-snapshot-metadata/pull/228), [@Priyankasaggu11929](https://github.com/Priyankasaggu11929))

## Dependencies

### Added
- k8s.io/streaming: v0.36.1

### Changed
- cel.dev/expr: v0.25.1 → v0.25.2
- github.com/GoogleCloudPlatform/opentelemetry-operations-go/detectors/gcp: [v1.30.0 → v1.32.0](https://github.com/GoogleCloudPlatform/opentelemetry-operations-go/compare/detectors/gcp/v1.30.0...detectors/gcp/v1.32.0)
- github.com/cncf/xds/go: [ee656c7 → dba9d58](https://github.com/cncf/xds/compare/ee656c7...dba9d58)
- github.com/coreos/go-oidc: [v2.3.0+incompatible → v2.5.0+incompatible](https://github.com/coreos/go-oidc/compare/v2.3.0...v2.5.0)
- github.com/envoyproxy/go-control-plane/envoy: [v1.36.0 → v1.37.0](https://github.com/envoyproxy/go-control-plane/compare/envoy/v1.36.0...envoy/v1.37.0)
- github.com/envoyproxy/protoc-gen-validate: [v1.3.0 → v1.3.3](https://github.com/envoyproxy/protoc-gen-validate/compare/v1.3.0...v1.3.3)
- github.com/fsnotify/fsnotify: [v1.9.0 → v1.10.1](https://github.com/fsnotify/fsnotify/compare/v1.9.0...v1.10.1)
- github.com/fxamacker/cbor/v2: [v2.9.0 → v2.9.2](https://github.com/fxamacker/cbor/compare/v2.9.0...v2.9.2)
- github.com/go-jose/go-jose/v4: [v4.1.3 → v4.1.4](https://github.com/go-jose/go-jose/compare/v4.1.3...v4.1.4)
- github.com/go-openapi/jsonpointer: [v0.22.5 → v0.24.0](https://github.com/go-openapi/jsonpointer/compare/v0.22.5...v0.24.0)
- github.com/go-openapi/jsonreference: [v0.21.5 → v0.21.6](https://github.com/go-openapi/jsonreference/compare/v0.21.5...v0.21.6)
- github.com/go-openapi/swag/cmdutils: [v0.25.5 → v0.27.0](https://github.com/go-openapi/swag/compare/cmdutils/v0.25.5...cmdutils/v0.27.0)
- github.com/go-openapi/swag/conv: [v0.25.5 → v0.27.0](https://github.com/go-openapi/swag/compare/conv/v0.25.5...conv/v0.27.0)
- github.com/go-openapi/swag/fileutils: [v0.25.5 → v0.27.0](https://github.com/go-openapi/swag/compare/fileutils/v0.25.5...fileutils/v0.27.0)
- github.com/go-openapi/swag/jsonname: [v0.25.5 → v0.26.0](https://github.com/go-openapi/swag/compare/jsonname/v0.25.5...jsonname/v0.26.0)
- github.com/go-openapi/swag/jsonutils/fixtures_test: [v0.25.5 → v0.27.0](https://github.com/go-openapi/swag/compare/jsonutils/fixtures_test/v0.25.5...jsonutils/fixtures_test/v0.27.0)
- github.com/go-openapi/swag/jsonutils: [v0.25.5 → v0.27.0](https://github.com/go-openapi/swag/compare/jsonutils/v0.25.5...jsonutils/v0.27.0)
- github.com/go-openapi/swag/loading: [v0.25.5 → v0.27.0](https://github.com/go-openapi/swag/compare/loading/v0.25.5...loading/v0.27.0)
- github.com/go-openapi/swag/mangling: [v0.25.5 → v0.27.0](https://github.com/go-openapi/swag/compare/mangling/v0.25.5...mangling/v0.27.0)
- github.com/go-openapi/swag/netutils: [v0.25.5 → v0.27.0](https://github.com/go-openapi/swag/compare/netutils/v0.25.5...netutils/v0.27.0)
- github.com/go-openapi/swag/stringutils: [v0.25.5 → v0.27.0](https://github.com/go-openapi/swag/compare/stringutils/v0.25.5...stringutils/v0.27.0)
- github.com/go-openapi/swag/typeutils: [v0.25.5 → v0.27.0](https://github.com/go-openapi/swag/compare/typeutils/v0.25.5...typeutils/v0.27.0)
- github.com/go-openapi/swag/yamlutils: [v0.25.5 → v0.27.0](https://github.com/go-openapi/swag/compare/yamlutils/v0.25.5...yamlutils/v0.27.0)
- github.com/go-openapi/swag: [v0.25.5 → v0.27.0](https://github.com/go-openapi/swag/compare/v0.25.5...v0.27.0)
- github.com/go-openapi/testify/enable/yaml/v2: [v2.4.0 → v2.6.0](https://github.com/go-openapi/testify/compare/enable/yaml/v2/v2.4.0...enable/yaml/v2/v2.6.0)
- github.com/go-openapi/testify/v2: [v2.4.0 → v2.6.0](https://github.com/go-openapi/testify/compare/v2.4.0...v2.6.0)
- github.com/golang-jwt/jwt/v5: [v5.3.0 → v5.3.1](https://github.com/golang-jwt/jwt/compare/v5.3.0...v5.3.1)
- github.com/google/cel-go: [v0.27.0 → v0.28.1](https://github.com/google/cel-go/compare/v0.27.0...v0.28.1)
- github.com/google/pprof: [27863c8 → 40e02aa](https://github.com/google/pprof/compare/27863c8...40e02aa)
- github.com/grpc-ecosystem/go-grpc-middleware/providers/prometheus: [v1.0.1 → v1.1.0](https://github.com/grpc-ecosystem/go-grpc-middleware/compare/providers/prometheus/v1.0.1...providers/prometheus/v1.1.0)
- github.com/grpc-ecosystem/go-grpc-middleware/v2: [v2.3.0 → v2.3.3](https://github.com/grpc-ecosystem/go-grpc-middleware/compare/v2.3.0...v2.3.3)
- github.com/grpc-ecosystem/grpc-gateway/v2: [v2.27.7 → v2.29.0](https://github.com/grpc-ecosystem/grpc-gateway/compare/v2.27.7...v2.29.0)
- github.com/kubernetes-csi/csi-lib-utils: [v0.23.2 → v0.24.0](https://github.com/kubernetes-csi/csi-lib-utils/compare/v0.23.2...v0.24.0)
- github.com/kubernetes-csi/external-snapshotter/client/v8: [v8.4.0 → v8.6.0](https://github.com/kubernetes-csi/external-snapshotter/compare/client/v8/v8.4.0...client/v8/v8.6.0)
- github.com/kubernetes-csi/external-snapshotter/v8: [v8.5.0 → v8.6.0](https://github.com/kubernetes-csi/external-snapshotter/compare/v8.5.0...v8.6.0)
- github.com/moby/spdystream: [v0.5.0 → v0.5.1](https://github.com/moby/spdystream/compare/v0.5.0...v0.5.1)
- github.com/onsi/ginkgo/v2: [v2.27.2 → v2.22.0](https://github.com/onsi/ginkgo/compare/v2.27.2...v2.22.0)
- github.com/onsi/gomega: [v1.38.2 → v1.36.1](https://github.com/onsi/gomega/compare/v1.38.2...v1.36.1)
- github.com/prometheus/common: [v0.67.5 → v0.69.0](https://github.com/prometheus/common/compare/v0.67.5...v0.69.0)
- github.com/prometheus/procfs: [v0.20.1 → v0.21.1](https://github.com/prometheus/procfs/compare/v0.20.1...v0.21.1)
- go.etcd.io/etcd/api/v3: v3.6.7 → v3.6.11
- go.etcd.io/etcd/client/pkg/v3: v3.6.7 → v3.6.11
- go.etcd.io/etcd/client/v3: v3.6.7 → v3.6.11
- go.etcd.io/etcd/pkg/v3: v3.6.5 → v3.6.8
- go.etcd.io/etcd/server/v3: v3.6.5 → v3.6.8
- go.opentelemetry.io/contrib/detectors/gcp: v1.39.0 → v1.43.0
- go.opentelemetry.io/contrib/instrumentation/google.golang.org/grpc/otelgrpc: v0.67.0 → v0.69.0
- go.opentelemetry.io/contrib/instrumentation/net/http/otelhttp: v0.65.0 → v0.68.0
- go.opentelemetry.io/otel/exporters/otlp/otlptrace/otlptracegrpc: v1.40.0 → v1.43.0
- go.opentelemetry.io/otel/exporters/otlp/otlptrace: v1.40.0 → v1.43.0
- go.opentelemetry.io/otel/metric: v1.42.0 → v1.44.0
- go.opentelemetry.io/otel/sdk/metric: v1.42.0 → v1.44.0
- go.opentelemetry.io/otel/sdk: v1.42.0 → v1.44.0
- go.opentelemetry.io/otel/trace: v1.42.0 → v1.44.0
- go.opentelemetry.io/otel: v1.42.0 → v1.44.0
- go.opentelemetry.io/proto/otlp: v1.9.0 → v1.10.0
- go.uber.org/zap: v1.27.1 → v1.28.0
- golang.org/x/crypto: v0.48.0 → v0.53.0
- golang.org/x/mod: v0.32.0 → v0.37.0
- golang.org/x/net: v0.51.0 → v0.56.0
- golang.org/x/sync: v0.19.0 → v0.21.0
- golang.org/x/sys: v0.42.0 → v0.46.0
- golang.org/x/term: v0.40.0 → v0.44.0
- golang.org/x/text: v0.34.0 → v0.39.0
- golang.org/x/tools: v0.41.0 → v0.47.0
- gonum.org/v1/gonum: v0.16.0 → v0.17.0
- google.golang.org/genproto/googleapis/api: 8636f87 → afd174a
- google.golang.org/genproto/googleapis/rpc: a57be14 → 3dc84a4
- google.golang.org/grpc: v1.79.2 → v1.82.0
- google.golang.org/protobuf: v1.36.11 → f2248ac
- k8s.io/api: v0.35.0 → v0.36.1
- k8s.io/apiextensions-apiserver: v0.35.0 → v0.36.1
- k8s.io/apimachinery: v0.35.0 → v0.36.1
- k8s.io/apiserver: v0.35.0 → v0.36.1
- k8s.io/client-go: v0.35.0 → v0.36.1
- k8s.io/code-generator: v0.35.0 → v0.36.1
- k8s.io/component-base: v0.35.0 → v0.36.1
- k8s.io/component-helpers: v0.35.0 → v0.36.1
- k8s.io/kms: v0.35.0 → v0.36.1
- k8s.io/kube-openapi: 4e65d59 → 43fb72c
- k8s.io/utils: 914a6e7 → b8788ab
- sigs.k8s.io/apiserver-network-proxy/konnectivity-client: v0.34.0 → v0.35.0
- sigs.k8s.io/structured-merge-diff/v6: v6.3.2 → v6.4.0

### Removed
- github.com/Masterminds/semver/v3: [v3.4.0](https://github.com/Masterminds/semver/tree/v3.4.0)
- github.com/armon/go-socks5: [e753329](https://github.com/armon/go-socks5/tree/e753329)
- github.com/cenkalti/backoff/v4: [v4.3.0](https://github.com/cenkalti/backoff/tree/v4.3.0)
- github.com/gregjones/httpcache: [901d907](https://github.com/gregjones/httpcache/tree/901d907)
- github.com/grpc-ecosystem/go-grpc-prometheus: [v1.2.0](https://github.com/grpc-ecosystem/go-grpc-prometheus/tree/v1.2.0)
