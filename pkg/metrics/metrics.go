...
const (
	...
	// Specifically, the operation metric is emitted based on the following timestamps:
	// - Start_time: controller notices the first time that there is a GetMetadataDelta RPC call to fetch the changed blocks betweeen 2 snapshots
	...
	// MetadataStatusTypeUnknown represents that the status is unknown
	MetadataStatusTypeUnknown metadataStatusType = "unknown"
	// Success and Cancel are statuses for operation time (operation_total_seconds) as seen by snapshot metadata controller
	// MetadataStatusTypeSuccess represents that a GetMetadataAllocated, GetMetadataDelta
	// has finished successfully.
	// Individual reconciliations (reconciliation_total_seconds) also use this status.
	MetadataStatusTypeSuccess metadataStatusType = "success"
)
...
type MetricsManager interface {
	...
	PrepareMetricsPath(mux *http.ServeMux, pattern string) error
...
func (opMgr *operationMetricsManager) RecordMetrics(opKey OperationKey, opStatus OperationStatus, driverName string) {
	...
	status := string(MetadataStatusTypeUnknown)
	if opStatus != nil {
	...
	opMgr.opLatencyMetrics.WithLabelValues(driverName, opKey.Name, opVal.Driver, status).Observe(operationDuration)
...
func (opMgr *operationMetricsManager) PrepareMetricsPath(mux *http.ServeMux, pattern string) error {
...
// metadataStatusType represents the type of metadata RPC status to report
type metadataStatusType string

// MetadataOperationStatus represents the status for a metadata RPC operation
type MetadataOperationStatus struct {
	status metadataStatusType
}

// NewMetadataOperationStatus returns a new SnapshotOperationStatus
func NewMetadataOperationStatus(status metadataStatusType) MetadataOperationStatus {
	return MetadataOperationStatus{
		status: status,
	}
}

func (sos MetadataOperationStatus) String() string {
